...
/* Function to delete the node having given value */
node *delete (node *root, int element)
{
    node *temp;
    if (root == NULL) //if no tree present
    {
        return root;
    }

    /* we can directly delete leaf node from the tree by
    doing free of its space and giving null value to its parent */
    ...
    /*our element is present on left subtree */
    if (element < root->item)
    {
        root->left = delete (root->left, element);
    }

    /*our element is present on right subtree */
    else if (element > root->item)
    {
        root->right = delete (root->right, element);
    }

    /*deleting element which we want by replacing its value by its 
    inorder predecessor and then deleting its inorder predecessor*/
    else
    {
        temp = Predecessor(root, NULL, element);
        if (temp == NULL)
        {
            temp = Successor(root, NULL, element);
            root->item = temp->item;
...
int main()
{
    ...
    while (true)
    {
        ...
        if (n == 8) // delete the element
        {
            int element;
            scanf(" %d", &element);
            if (search(root, element) == false) //checking node is there or not
            {
